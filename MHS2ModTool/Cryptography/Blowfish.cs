...
namespace MHS2ModTool.Cryptography
{
    class Blowfish
    {
        ...
        public Blowfish(string key) : this(Encoding.UTF8.GetBytes(key))
        {
        }
        ...
        public Blowfish(ReadOnlySpan<byte> key)
        {
            ...
            for (int i = 0; i < 18; i++)
            {
                byte a = key[keyIndex++ % key.Length];
                byte b = key[keyIndex++ % key.Length];
                byte c = key[keyIndex++ % key.Length];
                byte d = key[keyIndex++ % key.Length];

                _p[i] ^= (uint)(a << 24 | b << 16 | c << 8 | d);
            }
        ...
        {
            byte[] output = new byte[(data.Length + 7) & ~7];

            ReadOnlySpan<uint> dataAsUint = MemoryMarshal.Cast<byte, uint>(data);
            Span<uint> outputAsUint = MemoryMarshal.Cast<byte, uint>(output);

            EncryptImpl(outputAsUint, dataAsUint[..(dataAsUint.Length & ~1)]);

            if (data.Length < output.Length)
            {
                // If the input is not 8 bytes aligned, let's pad it with zeros.
                Span<uint> temp = stackalloc uint[2];

                var remainder = data[(output.Length - 8)..];
                remainder.CopyTo(MemoryMarshal.Cast<uint, byte>(temp)[..remainder.Length]);

                EncryptImpl(outputAsUint[^2..], temp);
            }

            return output;
        }

        private void EncryptImpl(Span<uint> output, ReadOnlySpan<uint> data)
        {
        ...
        public byte[] Decrypt(ReadOnlySpan<byte> data)
        {
            ...
            DecryptImpl(outputAsUint, dataAsUint);

            return output;
        }

        private void DecryptImpl(Span<uint> output, ReadOnlySpan<uint> data)
        {
            for (int i = 0; i < data.Length; i += 2)
            {
                uint left = data[i];
        ...
        private void Encrypt(ref uint left, ref uint right)
        {
        ...
        private void Decrypt(ref uint left, ref uint right)
        {
        ...
        {
            uint h = _s[0][x >> 24] + _s[1][x >> 16 & 0xff];
            return (h ^ _s[2][x >> 8 & 0xff]) + _s[3][x & 0xff];
        }

        private static void Swap(ref uint left, ref uint right)
        {
